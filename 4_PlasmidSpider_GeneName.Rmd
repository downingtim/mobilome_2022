...
# Potential input files [1] - [17] read from the local folder
...
# [8]  Plasmid_PPI_data_PMNLE_corrected2.csv = 4,377 samples with their String ID,
...
# [9]  approximatePMNLE_Feb2022_v2.xlsx = like [8] but with 4,445 samples shown
#      where some contain no relevant data
# [10] approximatePMNLE_all.xlsx = like [8] but with 4,422 samples shown
#      where some contain no relevant data
# [11] approximatePMNLEmodified.xlsx  = like [8] but with 4,422 samples shown
#      where some contain no relevant data
# [12] Table_S3.xlsx = 4,377 samples with the plasmid-related 
...
#      (ccs_all)
# [13] OUTPUT7 -> points to chromosomal_PPIs_4377.tar.gz -> 4,377 samples' CSV
...
#      number of chromosomal PPIs (c) - for each sample
#      See it at Zenodo at https://doi.org/10.6084/m9.figshare.X
# [14] Data_for_Fig_S7_metasamples.csv = metasamples from NMF on plasmid gene
#      presence-absence matrix
# [15] Data_for_Fig_S11_metasamples.interactions.all.csv = mixture gene
#      coefficients from NMF on plasmid gene presence-absence matrix
# [16] Data_for_Fig_S8_mixture_coefficients.interactions.all.csv = metasamples
#      from NMF on plasmid gene PPI rates matrix
# [17] Data_for_Fig_S12_mixture_coefficients.interactions.all.csv = mixture 
...
fullPMNLE <- data.frame(read.csv("Plasmid_PPI_data_PMNLE_corrected2.csv"))
str(fullPMNLE) # 4377 obs. of  15 variables:
        # X1, STRING.bacteria.name, aPMNLE, cThreshold, cTriangles, cEdges
        # cVertices, cNonTrivialLoops, cConnectedComponents
        # add plasmid_output into Final_Table where species match
# fullPMNLE is not correct
...
setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/")
fullPMNLE_new <- data.frame(read.csv("Plasmid_PPI_data_PMNLE_corrected2.csv"))
str(fullPMNLE_new) # 4377 obs. of  15 variables:
...
sd(subset(fullPMNLE_new, Plasmid_PPI > 0)$Plasmid_PPI)
...
hist(log10(as.numeric(fullPMNLE_new$cTri)), breaks=123, col="grey",
     xlab="Log10-scaled numbers of all protein trios", main="", cex=1.2,
     cex.axis=1.4) + grid()
dev.off()
png("FigS11_Loops_distribution_full.png", width=500, height=600, units="px")
hist(log10(as.numeric(fullPMNLE_new$loops)), breaks=123, col="grey",
     xlab="Log10-scaled numbers of all indirect connections", main="",
     cex=1.2, cex.axis=1.4) + grid()
dev.off()
png("FigS12_Components_distribution_full.png", width=500, height=600, units="px")
hist(log10(fullPMNLE_new$ccs), breaks=123, col="grey",
     xlab="PPI network components - all genes", main="", cex=1.2,
     cex.axis=1.4) + grid()
dev.off()

cor.test(as.numeric(fullPMNLE_new$loops), fullPMNLE_new$aPMNLE)[4]
cor.test(as.numeric(fullPMNLE_new$loops), fullPMNLE_new$aPMNLE)[3]
cor.test(as.numeric(fullPMNLE_new$loops), fullPMNLE_new$aPMNLE)$conf.int[1:2]
# Loops: r=0.337 r2=0.114 raw p=2.568624e-113 95% CI 0.31 0.36 # p value 1
pvalues <- c(2.568624e-113)

cor.test(as.numeric(fullPMNLE_new$ccs), fullPMNLE_new$aPMNLE)[4]
cor.test(as.numeric(fullPMNLE_new$ccs), fullPMNLE_new$aPMNLE)[3]
cor.test(as.numeric(fullPMNLE_new$ccs), fullPMNLE_new$aPMNLE)$conf.int[1:2]
# Connected Components: r=-0.191 r2=0.001 raw p=4.585766e-36 95% CI -0.22 -0.16
pvalues <- c(4.585766e-36, pvalues) # p value 2
...
# Chrom aPMNLE
setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021")
approximatePMNLE <- data.frame(read.xlsx("approximatePMNLE_Feb2022_v2.xlsx", sheetIndex=1))
str(approximatePMNLE) # 4445 obs. of  5 variables:
        # X1, STRING.bacteria.name, aPMNLE, cThreshold, cTriangles, cEdges
        # cVertices, cNonTrivialLoops, cConnectedComponents
        # add plasmid_output into Final_Table where species match
...
png("FigS10_Triangles_distribution_chrom.png", width=500, height=600, units="px")
hist(log10(as.numeric(approximatePMNLE$cTri)), breaks=123, col="grey",
     ...
     cex.axis=1.4) + grid()
dev.off()
png("FigS11_Loops_distribution_chrom.png", width=500, height=600, units="px")
hist(log10(as.numeric(approximatePMNLE$loops)), breaks=123, col="grey",
     ...
     cex=1.2, cex.axis=1.4) + grid()
dev.off()
png("FigS12_Components_distribution_chrom.png", width=500, height=600, units="px")
...
str(approximatePMNLE_all) # 4422 obs. of  5 variables:
# X1 ID	STRING.bacteria.name	aPMNLE	cThreshold	cTriangles	cEdges	cVertices
...
View(approximatePMNLE_all) # All aPMNLE

png("FigS10_Triangles_distribution_all.png", width=500, height=600, units="px")
hist(log10(as.numeric(approximatePMNLE_all$cTri)), breaks=123, col="grey",
     xlab="Log10-scaled numbers of all protein trios", main="", cex=1.2,
     cex.axis=1.4) + grid()
dev.off()
png("FigS11_Loops_distribution_all.png", width=500, height=600, units="px")
hist(log10(as.numeric(approximatePMNLE_all$loops)), breaks=123, col="grey",
     xlab="Log10-scaled numbers of all indirect connections", main="",
     cex=1.2, cex.axis=1.4) + grid()
dev.off()
png("FigS12_Components_distribution_all.png", width=500, height=600, units="px")
...
# Loops: r=0.352 r2=0.124 raw p=1.53e-128 95% CI 0.33 0.38
pvalues <- c(1.53e-12, pvalues) # p value 3
...
# Connected Components: r=-0.200 r2=0.040 raw p=4.65e-41 95% CI -0.23 -0.17
pvalues <- c(4.65e-41, pvalues) # p value 4
...
Plasmid_PPI_data <- read.xlsx("Key_Results_Figures/Table_S3.xlsx")
...
for (k in 1:length(Plasmid_PPI_data$Sample)){
  print(k)
  for (i in 1:length(approximatePMNLE$STRINGbacterianame)){ # 
    Plasmid_PPI_data[k,]$Sample <- sub("\\_$", "", Plasmid_PPI_data[k,]$Sample) # remove trailing _  # "Escherichia_coli_O157_H7_str_EDL933"
    
    if(approximatePMNLE$STRINGbacterianame[i] == Plasmid_PPI_data[k,]$Sample){    
...
# Plasmid_PPI r=0.50 r2=0.25 p=2.542212e-185 95% CI = 0.47 to 0.53
pvalues <- c(2.542212e-185, pvalues) # p value 5
...
# Plasmid_PPI r=0.30 r2=0.09 p=2.542212e-185 95% CI = 0.26 to 0.33
pvalues <- c(2.542212e-185, pvalues) # p value 6
...
# Plasmid_PPI r=-0.411 r2=0.169 p=1.084262e-119 95% CI = -0.44 to -0.38
pvalues <- c(1.084262e-119, pvalues) # p value 7
...
# Chrom_PPI r=0.477 r2=0.229 p=9.165366e-49 95% CI = 0.23 to 0.30
pvalues <- c(9.165366e-49, pvalues) # p value 8
...
# Chrom_PPI r=0.131 r2=0.02 p=9.132868e-11 95% CI = 0.08 to 0.15
...
# Chrom_PPI r=-0.469 r2=0.220 p=2.877624e-160 95% CI = -0.50 to -0.44
pvalues <- c(2.877624e-160, pvalues) # p value 10
...
# interactions r=0.28 r2=0.08 p=5.771313e-55 95% CI = 0.25 to 0.315
...
# interactions r=-0.43 r2=0.184 p=5.279136e-131 95% CI = -0.46 to -0.40
pvalues <- c(5.279136e-131, pvalues) # p value 13
# # ####
...
# inputProteins r=0.42 r2=0.18 p=2.505911e-75 95% CI = 0.30 to 0.36
pvalues <- c(2.505911e-75, pvalues) # p value 14
...
# inputProteins r=0.216 r2=0.05 p=6.181867e-24 95% CI = 0.15 to 0.22
pvalues <- c(6.181867e-24, pvalues) # p value 15
...
# inputProteins r=-0.365 r2=0.13 p=1.421257e-51 95% CI = -0.31 to -0.24
pvalues <- c(1.421257e-51, pvalues) # p value 16
###################
#################################################################
...
sd(Plasmid_PPI_dat3$diffPMNLE) # SD = 0.0704
#     Min.   1st Qu.    Median      Mean  3rd Qu.  Max 
#     -0.152   -0.0189    0.007    0.022  0.046  0.763 
 subset(Plasmid_PPI_dat3, diffPMNLE > 0.5)[c(1:3,10:13)]

str(Plasmid_PPI_dat3)  # 2975 obs. of  16 variables:
t3 <- subset(Plasmid_PPI_dat3, diffPMNLE >  2*sd(Plasmid_PPI_dat3$diffPMNLE) )
...
write.xlsx(t3, "Table_S4.xlsx")

t4 <- subset(Plasmid_PPI_dat3, diffPMNLE < -2*sd(Plasmid_PPI_dat3$diffPMNLE) )
...
par(mfrow=c(5,3))
pdf("mainset_v_155.pdf", width=6, height=6, bg="white")
p_set <- c()
t_set <- c()
for (i in 2:16){
   t3[,i] <- as.numeric(t3[,i])
   ttt <- t.test(as.numeric(t3[,i]), as.numeric(main_set2[,i]))
...
# compare groups' features
par(mfrow=c(3,1))
pdf("mainset_v_155.2.pdf", width=6, height=6, bg="white")
...
text(0.466,0.64, paste0("r=", round(cor(main_set2$aPMNLE, main_set2$aPMNLE_all),3),
    " and r2=", round(cor(main_set2$aPMNLE, main_set2$aPMNLE_all)**2,3), sep=""),
     col=alpha("blue",0.7))
...
text(1.6,2.6, "Samples with more connected", col=alpha("black",0.7), cex=1.1)
...
#     Min.   1st Qu.    Median      Mean   3rd Qu.      Max.      NA's 
#  0.008857 0.401626 0.510176 0.518688 0.629475 0.998985
...
# diffPMNLE vs diffLoops r=0.60 r2=0.36 p=5.50e-290 95% CI = 0.58 to 0.62
pvalues <- c(5.50e-290, pvalues) # p value 17
...
sd(Plasmid_PPI_dat3$diffTrios) # SD = 
#     Min.   1st Qu.    Median      Mean   3rd Qu.      Max.     
#    0.04    0.62     0.76      0.73      0.86      1.00000
...
cor(Plasmid_PPI_dat3$diffPMNLE, Plasmid_PPI_dat3$diffTrios)
...
sd(Plasmid_PPI_dat3$diffCcs) # SD = 
str(Plasmid_PPI_dat3$diffCcs) # 4422 - 126 samples = 4,398 samples
#     Min.   1st Qu.    Median      Mean   3rd Qu.      Max.      NA's 
#       -31     -1.1429  -0.6429    -Inf -0.3684  0.0000 
...
# diffPMNLE vs diffCcs r=-0.425 r2=0.18 p=1.53e-130 95% CI = -0.45 to 0.40
pvalues <- c(1.53e-130, pvalues) # p value 19
