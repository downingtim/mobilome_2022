...
dim(new)[1] # 494 obs. of  3023 variables:

mtrx <- matrix(unlist(new), ncol =3023, nrow = 494)
...
out2 <- cosine(mtrx)
str(out2)
rownames(out2) <- rownames(new) # 
colnames(out2) <- rownames(new) # 494
pdf("Figure_cosine_genes.pdf", width=46, height=46, bg="white")
...
length(subset(Gene_rates, Plasmid_genes>0)$Sample) # 491
...
fullPMNLE$STRING.bacteria.name[3232] = "Pseudomonas_sp_URMO17WK12I8" # fix names!!
...
setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/")
sample_names <- data.frame(read.csv("sample_names.csv"))
...
View(t(output1))
mean(main_set2$Chrom_genes + main_set2$Plasmid_genes) # 4013.2
mean(t3$Chrom_genes + t3$Plasmid_genes) # 2,314 
sd(main_set2$Chrom_genes + main_set2$Plasmid_genes) # 1336
sd(t3$Chrom_genes + t3$Plasmid_genes) # 1429
...
summary(fullPMNLE4$indirect_all )   # mean=0.9+-0.6
sd(na.omit(fullPMNLE4$indirect_all )  )

summary(fullPMNLE4$indirect_chrom )   # mean=3.7+-2.1
...
length(subset(fullPMNLE4, loops < loops_all)$loops) # 8
length(subset(fullPMNLE4, loops > loops_all)$loops)  # 16 equal, 439

summary(fullPMNLE$loops/(fullPMNLE$Chrom_genes)) # 5.5+-3.1
sd(na.omit(fullPMNLE$loops/(fullPMNLE$Chrom_genes)))

summary(fullPMNLE4$loops_all )  #  3136+-1452
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #   725    2090    2917    3136    3870   14062       4 
sd(na.omit(fullPMNLE4$loops_all ) ) # 1452
summary(fullPMNLE4$loops ) # 15668+-12178
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  14    7413   12718   15668   20554  118041      28 
sd(na.omit(fullPMNLE4$loops) ) # 12178
summary(fullPMNLE$loops ) # 21482+-18510
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
...
par(mfrow=c(1,3))  
hist( ((fullPMNLE$Plasmid_PPI)/fullPMNLE$Plasmid_genes), breaks=121, col="green", 
      main="", cex.lab=1.3, ylab="Number of plasmid-related proteins", xlim=c(0,120), density =42,
...
hist.data = hist(  (fullPMNLE$Plasmid_PPI/fullPMNLE$Plasmid_genes), plot=F, breaks=111)
hist.data$counts = as.numeric(log10(hist.data$counts )) 
 hist.data$counts[is.na(hist.data$counts)] <- 0
  hist.data$counts[is.infinite(hist.data$counts)] <- 0
...
sum(fullPMNLE$Plasmid_PPI) #   390,592
sum(fullPMNLE$Chrom_PPI)   # 286,364,425
sum(fullPMNLE$Plasmid_genes) #  13,348
sum(fullPMNLE$Chrom_genes)    # 15,450,151

fullPMNLE4 <- subset(fullPMNLE, Plasmid_genes>0)
summary(fullPMNLE4$Plasmid_genes)
sd(fullPMNLE4$Plasmid_genes) # median=5, mean=27.2+-112.3
# 1.00    2.00    5.00   27.19   14.00 1286.00 
summary(fullPMNLE4$Chrom_genes) # mean = 3927+-
sd(fullPMNLE4$Chrom_genes) # median=3923, mean = 3927+-1367
#   357    2900    3923    3927    4830    8564 
...
# median=0.16, mean=0.65+-2.50 percentages
# 0.01516  0.05929  0.15530  0.64503  0.37528 31.17576 

summary(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes) # 29.5+-  
sd( (fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes)) #  35.04
#  0.00   18.89   29.50   38.99   48.84  452.00      
summary(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes) #
sd(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes)   # 17.6+-5.7
# 1.00   14.03   16.40   17.64   19.74   78.28 
summary(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes) #
sd(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes)   # 14.8+-3.7

mean(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes)/ mean(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes) # ratio = 2.2105
...
subset(fullPMNLE4, rate>220)
hist(  log2(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes)/(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes), breaks=44, xlim=c(-1,2.5)) + grid()
...
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
 #    0    5957   14808   20047   27775  146082     125 
