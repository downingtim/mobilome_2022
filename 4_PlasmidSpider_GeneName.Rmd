...
# [2]  OUTPUT7.tar.gz -> 4,363 samples' CSV files for all proteins showing the
#      index, gene name (Gene), plasmid status (Plasmid, where 0=plasmid-related),
#      and total number of PPIs (Interactions) - for each sample
#      Download at https://doi.org/10.6084/m9.figshare.19672527
# [3]  OUTPUT8.tar.gz -> 4,363 samples' CSV files for all proteins showing the
#      PPIs, associated genes and StringDB identifiers
#      Download at shorturl.at/ - fix Drive link
# [4]  Table_3_new.xlsx = 4,363 samples' key summary data, with the plasmid-related 
...
#      (ccs_all)
# [5]  plasmid_list_1.xlsx contains the 16,393 unique plasmids, along with their
#      accessions, numbers of genes, plasmid name, sample name, strain name,
#      numbers of unique genes, numbers of CDSs, plasmid length, and list of genes.
# [6]  uniqueGeneList.csv = the 8,941 unique plasmid-encoded genes
# [7]  plasmidGenes.new.csv = matrix of 494 samples across 3,023 genes shown as
#      presence-absence (1 or 0)
# [8]  plasmidGenes.new.int.csv = matrix of 494 samples across 3,023 proteins 
#      shown as the number of PPIs per protein
...
str(vector2) # very loooong 14617
#View(vector2)
...
new <- data.frame(read.csv("plasmidGenes.new.csv"))   
rownames(new) <- new[,1]
new <- new[-c(1)] # drop unneeded column
dim(new)[1] # 364 obs. of  3023 variables:
...
yy <- t(new[1211,])
length(yy[yy>0]) # genes on plasmids  2921
...
str(uniqueGeneList) # 3432

mtrx <- matrix(unlist(new), ncol =2921, nrow = 364)
str(mtrx) # cosine similarity
out1 <- cosine(mtrx)
str(out1)
rownames(out1) <- colnames(new)
colnames(out1) <- colnames(new)
out1[is.nan(out1)] <- 0 # replace NaN
pdf("Figure_cosine_genes.pdf", width=86, height=86, bg="white")
heatmap.2(as.matrix(out1), dendrogram="both", trace="none",  
          margin=c(6,6), cex.lab=0.1, cex.axis=0.1, col=redblue(100))
dev.off()

mtrx2 <- matrix(unlist(new), ncol =3023, nrow = 494)
mtrx2 <- t(mtrx2) # transpose!
str(mtrx2) # cosine similarity
out2 <- cosine(mtrx)
str(out2)
rownames(out2) <- colnames(new) # 
colnames(out2) <- colnames(new) # 494
out2[is.nan(out2)] <- 0 # replace NaN
pdf("Figure_cosine_genes2.pdf", width=46, height=46, bg="white")
heatmap.2(as.matrix(out2), dendrogram="both", trace="none",  
          margin=c(6,6), cex.lab=0.1, cex.axis=0.1, col=redblue(100))
dev.off()

hist(  (rowSums(new)), breaks=199, xlim=c(0,22)) + grid()
hist(  (colSums(new)), breaks=99) + grid()
colSums1 <- colSums(new)
rowSums1 <- rowSums(new)
median(colSums1[colSums1 >0]) # 4 median number of samples in which gene found
IQR(colSums1[colSums1 >0])  # 3 IQR of number of samples in which gene is found
median(rowSums1[rowSums1 >0]) # 14 median genes per sample
IQR(rowSums1[rowSums1 >0])    # 26.5 IQR of number of genes per sample
summary(rowSums(new)) # mean = 12.14 +- 50.6
sd(rowSums(new)) 
summary(colSums(new)) #  
sd(colSums(new)) # mean = 1.98 +- 4.0
View(new)
...
str(Gene_rates) # 4432 samples 
names3 <- list.files(pattern = "data\\.csv$")
length(names3) # 4432 samples
for (k in 1:length(names3)){ # read in 4432 files
     name <- names3[k]
...
View(Gene_rates)
write.csv(Gene_rates, "../Gene_rates_new2.csv")
setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/RMD_FILES")
# Gene_rates <- Gene_rates[,-c(1)]
str(Gene_rates) # 4432 samples 
summary(Gene_rates$Plasmid_genes/Gene_rates$Chrom_genes) # mean = 0.0663%
sd(Gene_rates$Plasmid_genes/Gene_rates$Chrom_genes)  # SD = 1.04%
summary(Gene_rates$Chrom_genes)  # median = 3,316, mean =3,533
sd(Gene_rates$Chrom_genes) # SD=1,653
summary(Gene_rates$Plasmid_genes)  # mean = 2.4 median=0
sd(Gene_rates$Plasmid_genes)  # SD = 33.3
...
length(subset(Gene_rates, Plasmid_genes>0)$Sample) # 365

#Gene_rates <- data.frame(read.csv("Gene_rates_new.csv"))
#Gene_rates <- Gene_rates[,-c(1)]
str(Gene_rates) # 4432 # Sample, Plasmid_genes, Chrom_genes
...
str(fullPMNLE) # 4363 obs. of  23 variables
...
fullPMNLE <- fullPMNLE[order(fullPMNLE$STRING.bacteria.name),]
Gene_rates <- Gene_rates[order(Gene_rates$Sample),]
  ...
  for (kk in 1:length(Gene_rates$Sample)){
    ...
    if(fullPMNLE$STRING.bacteria.name[k] == Gene_rates$Sample[kk]){
       ...
       fullPMNLE$inputProteins[k] <- (Gene_rates$Plasmid_genes[kk] + 
                                       Gene_rates$Chrom_genes[kk]) } # end for
      print(k)
      print(fullPMNLE$STRING.bacteria.name[k]) }
} # for each entry
length(subset(fullPMNLE, Plasmid_genes >0)$Plasmid_genes) # 370
...
str(names4) # 4432
names4[1] = "Nostoc_azollae_0708__chrom_data.csv"
fullPMNLE$STRING.bacteria.name[1] = "Nostoc_azollae_0708" # remove '
...
for (k in 1:length(names4)){ 
     ...
     for (kk in 1:length(fullPMNLE$STRING.bacteria.name)){
       if(name == fullPMNLE$STRING.bacteria.name[kk]){
         print(fullPMNLE$STRING.bacteria.name[kk])
...
str(fullPMNLE) # 4363
length(subset(fullPMNLE, Plasmid_genes >0)$Plasmid_genes) # 370
subset(fullPMNLE, Plasmid_genes >0 & fullPMNLE$Plasmid_PPI<1) # has no PPIs
...
#setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/")
#ample_names <- data.frame(read.csv("sample_names.csv"))
#sample_names <- sample_names[,-c(1)]
#str(sample_names) # 491 genes with plasmid genes

for (kk in 1:length(sample_names$Sample)){
     sample_names$Sample[kk] = gsub("\\.(?=[^.]*\\.)", "", sample_names$Sample[kk], perl=T)
     sample_names$Sample[kk] = gsub("\\:", "",     sample_names$Sample[kk], perl=T)
     sample_names$Sample[kk] = gsub("\\[|\\]", "", sample_names$Sample[kk], perl=T)
     sample_names$Sample[kk] = gsub("\'", "",      sample_names$Sample[kk], perl=T)
     sample_names$Sample[kk] = gsub("\\.", "",      sample_names$Sample[kk], perl=T)
     sample_names$Sample[kk] = gsub('_{1}$', '',   sample_names$Sample[kk], perl=T) }
...
for (j in 1:length(approximatePMNLE$STRING.bacteria.name)){
     approximatePMNLE$STRING.bacteria.name[j] = gsub("\\.(?=[^.]*\\.)", "",
                          approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub("\\:", "",
                          approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub("_chrom_data.csv", "",
                          approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub("\\[|\\]", "",
                         approximatePMNLE$STRING.bacteria.name[j] , perl=T)
   approximatePMNLE$STRING.bacteria.name[j]  = gsub("\\.", "",
                          approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub('_{1}$', '',
                        approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub("_=_", "_",
                      approximatePMNLE$STRING.bacteria.name[j] , perl=T)
     approximatePMNLE$STRING.bacteria.name[j]  = gsub("\'", "",
                        approximatePMNLE$STRING.bacteria.name[j] , perl=T)
...
#for (k in 1:length(fullPMNLE$STRING.bacteria.name)){
  ...
  for (kk in 1:length(sample_names$Sample)){
     if(fullPMNLE$STRING.bacteria.name[k] == sample_names$Sample[kk]){
...
write.csv(fullPMNLE, "Plasmid_PPI_data_PMNLE.csv")
fullPMNLE <- data.frame(read.xlsx("Table_S3_new.xlsx")) # , sheetIndex = 1)
str(fullPMNLE)

length(subset(fullPMNLE, Plasmid_Plasmid_PPI > 0)$Plasmid_Plasmid_PPI) # 247
summary(subset(fullPMNLE, Plasmid_Plasmid_PPI > 0)$Plasmid_Plasmid_PPI)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    1.0     2.0     6.0   183.9    25.0 12074.0 

length(subset(fullPMNLE, Plasmid_PPI > 0)$Plasmid_PPI) # 368
summary(subset(fullPMNLE, Plasmid_PPI > 0)$Plasmid_PPI)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#      1.00    73.75   210.00  1061.40   689.25 37844.00 
sd(subset(fullPMNLE, Plasmid_PPI > 0)$Plasmid_PPI) # 3231.088
summary(subset(fullPMNLE, Plasmid_PPI > 0)$Chrom_PPI)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    8036   37859   57168   62022   77302  216372 
sd(subset(fullPMNLE, Plasmid_PPI > 0)$Chrom_PPI) # 32389.7

length(subset(fullPMNLE, Chrom_PPI > 0)$Chrom_PPI) # 4363
summary(subset(fullPMNLE, Chrom_PPI > 0)$Chrom_PPI)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  3235   33803   52520   65813   80812  517363 
sd(subset(fullPMNLE, Chrom_PPI > 0)$Chrom_PPI) # 47088.61

tt <-(subset(fullPMNLE, Plasmid_PPI > 0))
summary(tt$Plasmid_Plasmid_PPI + tt$Plasmid_PPI) # +- 3592.446
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  1      74     212    1185     696   49918 

summary((fullPMNLE$Plasmid_Plasmid_PPI)/(fullPMNLE$Plasmid_Plasmid_PPI + 
                        fullPMNLE$Plasmid_PPI + fullPMNLE$Chrom_PPI)) # 0.0143% 
summary((fullPMNLE$Plasmid_PPI)/(fullPMNLE$Plasmid_Plasmid_PPI + 
                        fullPMNLE$Plasmid_PPI + fullPMNLE$Chrom_PPI)) # 0.131% 
summary((fullPMNLE$Plasmid_PPI + fullPMNLE$Plasmid_Plasmid_PPI)/
          (fullPMNLE$Plasmid_Plasmid_PPI + 
                        fullPMNLE$Plasmid_PPI + fullPMNLE$Chrom_PPI)) # 0.145% 
summary(fullPMNLE$Fraction)
sum(fullPMNLE$Chrom_PPI) #  287143836
sum(fullPMNLE$Plasmid_Plasmid_PPI) # 45433
sum(fullPMNLE$Plasmid_PPI) # 390594

 cor ( (fullPMNLE$Plasmid_PPI + fullPMNLE$Plasmid_Plasmid_PPI),
              (fullPMNLE$Chrom_genes + fullPMNLE$Plasmid_genes)) # r=0.047

fullPMNLE$inputProteins = fullPMNLE$Chrom_genes + fullPMNLE$Plasmid_genes 
fullPMNLE_new <- subset(fullPMNLE, Plasmid_PPI >0)
str(fullPMNLE_new) # 368 obs. of  24 variables:
r2d <- cor (log10(fullPMNLE_new$Plasmid_PPI + fullPMNLE_new$Plasmid_Plasmid_PPI),
             log10(fullPMNLE_new$inputProteins)) # r=0.29
...
########################################################################
########################################################################
fullPMNLE$interactions = fullPMNLE$Chrom_PPI + fullPMNLE$Plasmid_PPI + fullPMNLE$Plasmid_Plasmid_PPI
rrr = cor.test(log10(fullPMNLE$inputProteins), y=log10(fullPMNLE$interactions))$estimate
rrr # r =0.914
pdf("Figure_2_revised2.pdf", width=6, height=6)
ggplot(fullPMNLE, aes(x= log10(fullPMNLE$inputProteins), y=log10(fullPMNLE$interactions))) +
  geom_point(color ="blue", size=0.2, shape=1, (aes(alpha=0.2)), show.legend=F) +
  geom_smooth(method='lm', col="red") + 
    scale_x_continuous(name="Number of proteins per sample", limits =c(2.8,4) ,
                     labels=c("562","1000","1778","3162","5623","10000")) +
  scale_y_continuous(name="Number of PPIs per sample", limits =c(4,5.3),
                     labels=c("10000","31623","100000","316228"))  + 
  annotation_custom(grobTree(textGrob(paste("For ",length(fullPMNLE$STRING.bacteria.name),
    " bacterial samples",sep=""),x=0.64,y=.1,hjust=0, gp=gpar(col="black",fontsize=9)))) +
  annotation_custom(grobTree(textGrob(paste("All proteins adj_r=", round(rrr,3),sep=""),
  x=0.65, y=.075, hjust=0, gp=gpar(col="black", fontsize=9)))) +
  annotation_custom(grobTree(textGrob(paste("All proteins adj_r2=", round(rrr**2,3),sep=""),
  x=0.65, y=.05, hjust=0, gp=gpar(col="black", fontsize=9))))  
dev.off()
...
# write.xlsx(fullPMNLE, "Table_S3_new2.xlsx")
...
# interactions r=0.43  p=2.150176e-198  
...
t.test(fullPMNLE4$interactions, fullPMNLE5$interactions) # no diff 
...
r51 <- sqrt(summary(lm((fullPMNLE4$indirect_chrom) ~ log10(fullPMNLE4$interactions)))$adj.r.squared) # 
r61 <- sqrt(summary(lm((fullPMNLE4$indirect_all) ~ log10(fullPMNLE4$interactions)))$adj.r.squared)   # 
...
x5_a_1 <- ggplot(fullPMNLE4, aes(x=log10(fullPMNLE4$interactions),
  ...
  x=0.01,y=.86, hjust=0, gp=gpar(col="blue", fontsize=10)))) +
  annotation_custom(grobTree(textGrob(paste("All proteins adj_r2=", round(r61**2,3),sep=""),
  x=0.01,y=.83, hjust=0, gp=gpar(col="blue", fontsize=10))))  
...
x5_a_2 <- x5_a_1 + geom_point(data=fullPMNLE4,
  ...
  x=0.01,y=.76, hjust=0, gp=gpar(col="red", fontsize=10)))) 
...
####
dim(subset(fullPMNLE, indirect_all >= 7)) # 1303
dim(subset(fullPMNLE, indirect_all <= 7 & indirect_all > 1.5)) # 2311
dim(subset(fullPMNLE, indirect_all <= 1.5)) # 624
dim(subset(fullPMNLE, indirect_chrom >= 7)) # 1328
dim(subset(fullPMNLE, indirect_chrom < 7)) # 2886
fullPMNLE_high <- subset(fullPMNLE4, indirect_all > 7 ) #  
fullPMNLE_mid <- subset(fullPMNLE, indirect_all <= 7 & indirect_all > 1.5 )  
fullPMNLE_low <- subset(fullPMNLE, indirect_all < 1.5)  

r21 <- sqrt(summary(lm((fullPMNLE_high$indirect_all) ~ log10(fullPMNLE_high$interactions)))$adj.r.squared) # 0.35
r41 <- sqrt(summary(lm((fullPMNLE_mid$indirect_all) ~ log10(fullPMNLE_mid$interactions)))$adj.r.squared)   # 0.66
r51 <- sqrt(summary(lm((fullPMNLE_low$indirect_chrom) ~ log10(fullPMNLE_low$interactions)))$adj.r.squared) # 0.72
r61 <- sqrt(summary(lm((fullPMNLE_low$indirect_all) ~ log10(fullPMNLE_low$interactions)))$adj.r.squared)   # 0.098
...
pdf("Fig_5_IndirectConnections2.pdf", width=12, height=6, bg="white")
...
figure5_d1 <- ggExtra::ggMarginal(figure5_d_2, type="densigram",
                    size=5,colour="grey", bins=50)  
par(mfrow=c(2,2))
pdf("Fig_S_spare.pdf", width=10, height=12, bg="white")
ggarrange(figure5_a1, figure5_b1, figure5_c1, figure5_d1, ncol=2, nrow=2)
dev.off()
...
fullPMNLE4$proteins_ccs_all = log10(fullPMNLE4$inputProteins/fullPMNLE4$ccs_all)
fullPMNLE4$proteins_ccs_chrom = log10(fullPMNLE4$Chrom_genes/fullPMNLE4$ccs)
fullPMNLE4[mapply(is.infinite, fullPMNLE4)] <- NA

r1 <- cor.test(fullPMNLE4$proteins_ccs_chrom, log10(fullPMNLE4$interactions))$estimate
r2 <- cor.test(fullPMNLE4$proteins_ccs_all, log10(fullPMNLE4$interactions))$estimate

figure5_a_1 <- ggplot(fullPMNLE4, aes(x=log10(fullPMNLE4$interactions),
                                          y=fullPMNLE4$proteins_ccs_all)) +
  geom_point(color ="black", size=0.1, shape=1, (aes(alpha=0.1)), show.legend=F) +
  scale_x_continuous(name="Log10 of number of PPIs per sample", limits =c(4.5,5.4) ) +
  scale_y_continuous(name="Proteins per connected component", limits =c(1,2.5))  + 
  geom_smooth(method='lm', col="black") + 
  annotation_custom(grobTree(textGrob(paste("For ",length(fullPMNLE4$STRING.bacteria.name),
...
figure5_a_2 <- figure5_a_1 + geom_point(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$interactions), y=fullPMNLE4$proteins_ccs_chrom, alpha=0.1),
             color ="red", size=0.1, shape=2, show.legend=F) + 
  geom_smooth(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$interactions), y=fullPMNLE4$proteins_ccs_chrom),
        method='lm', col="red") +
...
### Proteins
r1p <- cor.test(fullPMNLE4$proteins_ccs_chrom, log10(fullPMNLE4$inputProteins))$estimate
r2p <- cor.test(fullPMNLE4$proteins_ccs_all, log10(fullPMNLE4$inputProteins))$estimate

figure5_b_1 <- ggplot(fullPMNLE4, aes(x=log10(fullPMNLE4$inputProteins),
                                          y=fullPMNLE4$proteins_ccs_all)) +
  geom_point(color ="black", size=0.1, shape=1, (aes(alpha=0.1)), show.legend=F) +
  scale_x_continuous(name="Log10 of number of proteins per sample", limits =c(3.1,3.9)) +
  scale_y_continuous(name="Proteins per connected component", limits =c(1,2.5))  + 
  geom_smooth(method='lm', col="black") + 
  annotation_custom(grobTree(textGrob(paste("For ",length(fullPMNLE4$STRING.bacteria.name),
    " bacterial samples",sep=""),x=0.01,y=.89,hjust=0, gp=gpar(col="black",fontsize=7)))) +
...
figure5_b_2 <- figure5_b_1 + geom_point(data=fullPMNLE4,
    aes(x=log10(fullPMNLE4$inputProteins), y=fullPMNLE4$proteins_ccs_chrom, alpha=0.1),
             color ="red", size=0.1, shape=2, show.legend=F) + 
  geom_smooth(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$inputProteins), y=fullPMNLE4$proteins_ccs_chrom),
        method='lm', col="red") +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r=", round(r1p,3),sep=""),
  x=0.01,y=.83, hjust=0, gp=gpar(col="red", fontsize=7)))) +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r2=", round(r1p**2,3),sep=""),
...
r1c <- cor.test(fullPMNLE4$proteins_ccs_chrom, log10(fullPMNLE4$Chrom_PPI))$estimate
r2c <- cor.test(fullPMNLE4$proteins_ccs_all, log10(fullPMNLE4$Chrom_PPI))$estimate

figure5_c_1 <- ggplot(fullPMNLE4, aes(x=log10(fullPMNLE4$Chrom_PPI),
                                          y=fullPMNLE4$proteins_ccs_all)) +
  geom_point(color ="black", size=0.1, shape=1, (aes(alpha=0.1)), show.legend=F) +
  scale_x_continuous(name="Log10 of number of chromosomal PPIs per sample", limits =c(4.5,5.4)) +
  scale_y_continuous(name="Proteins per connected component", limits =c(1,2.5))  + 
  geom_smooth(method='lm', col="black") + 
  annotation_custom(grobTree(textGrob(paste("For ",length(fullPMNLE4$STRING.bacteria.name),
    " bacterial samples",sep=""),x=0.01,y=.89,hjust=0, gp=gpar(col="black",fontsize=7)))) +
...
figure5_c_2 <- figure5_c_1 + geom_point(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$Chrom_PPI), y=fullPMNLE4$proteins_ccs_chrom, alpha=0.1),
             color ="red", size=0.1, shape=2, show.legend=F) + 
  geom_smooth(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$Chrom_PPI), y=fullPMNLE4$proteins_ccs_chrom),
        method='lm', col="red") +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r=", round(r1c,3),sep=""),
  x=0.02, y=.83, hjust=0, gp=gpar(col="red", fontsize=7)))) +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r2=", round(r1c**2,3),sep=""),
...
### Plasmid_PPI
r1d <- cor.test(fullPMNLE4$proteins_ccs_chrom, log10(fullPMNLE4$Plasmid_PPI))$estimate
r2d <- cor.test(fullPMNLE4$proteins_ccs_all, log10(fullPMNLE4$Plasmid_PPI))$estimate

figure5_d_1 <- ggplot(fullPMNLE4, aes(x=log10(fullPMNLE4$Plasmid_PPI),
                                          y=fullPMNLE4$proteins_ccs_all)) +
  geom_point(color ="black", size=0.1, shape=1, (aes(alpha=0.1)), show.legend=F) +
  scale_x_continuous(name="Log10 of number of plasmid PPIs per sample", limits =c(1,2.5)) +
  scale_y_continuous(name="Proteins per connected component", limits =c(1,2.5))  + 
  geom_smooth(method='lm', col="black") + 
  annotation_custom(grobTree(textGrob(paste("For ",length(fullPMNLE4$STRING.bacteria.name),
    " bacterial samples",sep=""),x=0.01,y=.89,hjust=0, gp=gpar(col="black",fontsize=7)))) +
...
figure5_d_2 <- figure5_d_1 + geom_point(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$Plasmid_PPI), y=fullPMNLE4$proteins_ccs_chrom, alpha=0.1),
             color ="red", size=0.1, shape=2, show.legend=F) + 
  geom_smooth(data=fullPMNLE4,
        aes(x=log10(fullPMNLE4$Plasmid_PPI), y=fullPMNLE4$proteins_ccs_chrom),
        method='lm', col="red") +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r=", round(r1d,3),sep=""),
  x=0.02, y=.83, hjust=0, gp=gpar(col="red", fontsize=7)))) +
  annotation_custom(grobTree(textGrob(paste("Chrom proteins adj_r2=", round(r1d**2,3),sep=""),
...
figure5_d1 <- ggExtra::ggMarginal(figure5_d_2, type="densigram",
                    size=5,colour="grey", bins=50)  
par(mfrow=c(2,2))
pdf("Fig_S23_proteinsPerCC.pdf", width=10, height=12, bg="white")
ggarrange(figure5_a1, figure5_b1, figure5_c1, figure5_d1, ncol=2, nrow=2)
dev.off()
#png("Fig_5_PMNLE.png", width=2500, height=2600, units="px")
#ggarrange(figure5_a1, figure5_b1, figure5_c1, figure5_d1, ncol=2, nrow=2)
...
sd(na.omit(Plasmid_PPI_dat3$diffPMNLE)) # SD = 0.085
#     Min.   1st Qu.    Median      Mean  3rd Qu.  Max  NAs
# -0.114881 -0.003824  0.026258  0.044004  0.071057  0.762518        22 

dim(Plasmid_PPI_dat3)  # 489
t2 <- subset(Plasmid_PPI_dat3, diffPMNLE >  2*sd(na.omit(Plasmid_PPI_dat3$diffPMNLE)))
dim(t2)
t3 <- subset(t2, aPMNLE_all > 0)
...
View(t(output1))
mean(main_set2$Chrom_genes + main_set2$Plasmid_genes) # 3989.977
mean(t3$Chrom_genes + t3$Plasmid_genes) # 2121.846 
sd(main_set2$Chrom_genes + main_set2$Plasmid_genes) # 1332.169
sd(t3$Chrom_genes + t3$Plasmid_genes) # 1593.157
...
# Although most showed statistically extreme differences between the 13 and
...
sum(fullPMNLE4$Plasmid_Plasmid_PPI)/sum(fullPMNLE4$Plasmid_PPI+fullPMNLE4$Plasmid_Plasmid_PPI)

summary(fullPMNLE4$indirect_all )   # mean=0.9+-0.5
sd(na.omit(fullPMNLE4$indirect_all )  )

summary(fullPMNLE4$indirect_chrom )   # mean=3.5+-1.8
sd(na.omit(fullPMNLE4$indirect_chrom)  )

t.test(fullPMNLE4$indirect_chrom, fullPMNLE4$indirect_all )

summary(fullPMNLE4$loops_all/(fullPMNLE4$Chrom_genes+fullPMNLE4$Plasmid_genes)) # 0.8+-0.3
sd(na.omit(fullPMNLE4$loops_all/(fullPMNLE4$Chrom_genes+fullPMNLE4$Plasmid_genes)))

summary(fullPMNLE4$loops/(fullPMNLE4$Chrom_genes)) # 3.7+-2.1
sd(na.omit(fullPMNLE4$loops/(fullPMNLE4$Chrom_genes)))

summary(fullPMNLE$indirect_chrom )   # mean=5.5+-3.1
sd(na.omit(fullPMNLE$indirect_chrom)  )

t.test(fullPMNLE4$indirect_chrom, fullPMNLE$indirect_chrom )

summary(fullPMNLE4$loops/(fullPMNLE4$loops_all)) # 4.95+-3.02 (median=4.37)
sd(na.omit(fullPMNLE4$loops/(fullPMNLE4$loops_all))) # 
hist(fullPMNLE4$loops/(fullPMNLE4$loops_all), breaks=111) # 4.95+-3.02 (median=4.37)
length(subset(fullPMNLE4, loops < loops_all)$loops) # 176
length(subset(fullPMNLE4, loops > loops_all)$loops)  # 156

summary(fullPMNLE$loops/(fullPMNLE$Chrom_genes)) # 5.5+-3.1
sd(na.omit(fullPMNLE$loops/(fullPMNLE$Chrom_genes)))

summary(as.numeric(fullPMNLE4$loops_all ))  #  3141+-1379.974
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #      725    2103    2929    3141    3898    8957       4 
sd(na.omit(fullPMNLE4$loops_all ) ) # 1379.974
summary(as.numeric(fullPMNLE4$loops) ) # 14938+-10839.75
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #  14    6954   12158   14938   20249   62237      20 
sd(na.omit(fullPMNLE4$loops) ) # 10839.75
summary(as.numeric(fullPMNLE$loops) ) # 21482+-18510
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #    0    8074   16518   21482   29073  146082     149 
sd(na.omit(fullPMNLE$loops) ) # 18510
t.test(fullPMNLE4$loops,fullPMNLE4$loops_all )


summary(fullPMNLE4$Plasmid_Plasmid_PPI )
#   Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
 #   0.00     0.00     2.0   123.8    12.0 12074.0 

fullPMNLE4$loops <- as.numeric(fullPMNLE4$loops)
fullPMNLE4$loops_all <- as.numeric(fullPMNLE4$loops_all)
fullPMNLE4$Plasmid_PPI <- as.numeric(fullPMNLE4$Plasmid_PPI)
...
library(gplots)
dim(new) # 494 samples by 3023 genes
...
median(colSums(new_33)) # 0 median number of samples in which each gene is found
IQR(colSums(new_33))    # 1 IQR of number of samples in which each gene is found
median(rowSums(new_33)) # 0 median genes per sample
IQR(rowSums(new_33))    # 0 IQR of number of genes per sample

# E. coli
new_4 <- new[c(165:170,361:363),] # only E. coli
#new_4 <- subset(new_4, select = -c(rows1))
new_44 <- new_4 %>% select_if(colSums(.) > 0)
median(colSums(new_44)) # 6 median number of samples in which each gene is found
median(rowSums(new_44)) # 353 median genes per sample
summary(rowSums(new_44)) # 323 
...
png("Figure_S4_All_PCA_justEcoli.png", width=1311,height=911,
    bg="white", units="px")
  par(mfrow = c(1, 2))
...
png("Figure_S4_All_PCA.png", width=1311,height=911,
    bg="white", units="px")
  par(mfrow = c(1, 2))
...
png(file="All_PCA_1v2_AcrossGenes.png", width=pcvalue1hw*5,
...
hist(yy$PC1)  
str(rownames(subset(yy, PC1<1))) # 3023 genes
...
library(lsa)
dim(new)  # 494 samples x 3023 genes
mylsa <- lsa(new)

str(mylsa) # $tk = 1-494 samples (left singular vector)
# $dk 1-3023 genes (document vector) - variable data
# and diagnonal matrix $sk 1:360 - 360 components
# Can change Dk to other plasmid gene patterns
sum(mylsa$sk) # 214.0872
head(mylsa$sk) # 48.14294 30.42497 26.33247 17.66877 12.81105 12.04075
...
mylsa_t <- lsa(t(new))
mycol<- c(rainbow(dim(new)[1], start=0.1, end=1)) # group samples
...
names5 <- list.files(pattern="_genes_data.csv")
str(names5) # 4432
plasmidPPIs <- 0
chromPPIs <- 0
plasmidPPIs_genes <- 0
...
str(table2) # 4432 obs. of  7 variables:

setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/Key_Results_Figures/")
# write.csv(table2, "Plasmid_PPI_rates.csv")
#table2 <- data.frame(read.csv("Plasmid_PPI_rates.csv"))
#str(table2)
str(fullPMNLE)
...
fullPMNLE$Plasmid_PPI <- fullPMNLE$plasmidPPIs
fullPMNLE$Chrom_PPI <- fullPMNLE$chromPPIs

# you need to save the below manually due to plotting issues, sorry
pdf("Figure_4_Plasmid_Gene_Interaction_Rate_Distribution.pdf", width=11, height=5)
par(mfrow=c(1,3))  
plot1 <- as.numeric(fullPMNLE$Plasmid_PPI/fullPMNLE$Plasmid_genes)
plot1 <- ifelse(is.na(plot1), 0, plot1)
hist( plot1, breaks=31, col="green", xlim=c(15,120), density =42, ylim=c(0,61),
      main="", cex.lab=1.3, ylab="Number of plasmid-related proteins",
     xlab="Average number of PPIs per plasmid-related protein") + grid()
hist( (fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes), breaks=41, col="purple", main="",  
     ylab="Number of chromosomal proteins", xlim=c(0,120),  density =42, cex.lab=1.3,
     xlab="Average number of PPIs per chromosomal protein", ylim=c(0,940)) + grid()
hist.data = hist(  (fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes), plot=F, breaks=27)
hist.data$counts = as.numeric(log10(hist.data$counts )) 
 hist.data$counts[is.na(hist.data$counts)] <- 0
  hist.data$counts[is.infinite(hist.data$counts)] <- 0
plot( hist.data,   col="purple", main="",  xlim=c(0,120), # xaxt="n", yaxt="n",
     ylab="Log10-scaled number of proteins", density =42, cex.lab=1.3,
    xlab="Average number of PPIs per protein per sample", ylim=c(0,3.2)) + grid()
hist.data = hist(  (fullPMNLE$Plasmid_PPI/fullPMNLE$Plasmid_genes), plot=F,
                   breaks=71)
hist.data$counts = as.numeric(log10(hist.data$counts )) 
 hist.data$counts[is.na(hist.data$counts)] <- 0
  hist.data$counts[is.infinite(hist.data$counts)] <- 0
plot( hist.data,   col="green", add=T, cex.lab=1.3, density =34)
...
sum(fullPMNLE$Plasmid_genes) #  13348
sum(fullPMNLE$Chrom_genes)    # 15450151

fullPMNLE4 <- subset(fullPMNLE, Plasmid_genes>0)
summary(fullPMNLE4$Plasmid_genes)
sd(fullPMNLE4$Plasmid_genes) # median=5, mean=28.9+-112
# 1.00    2.00    5.00   28.89   15.00 1286.00 
summary(fullPMNLE4$Chrom_genes) # mean = 3927+-
sd(fullPMNLE4$Chrom_genes) # median=3956, mean = 3894+-1376
#   357    2815    3956    3894    4838    8564
100*summary(fullPMNLE4$Plasmid_genes/(fullPMNLE4$Chrom_genes+fullPMNLE4$Plasmid_genes))
100*sd(fullPMNLE4$Plasmid_genes/(fullPMNLE4$Chrom_genes+fullPMNLE4$Plasmid_genes))
# median=0.1558, mean=0.6988+-2.62 percentages
# 0.01516  0.05925  0.15579  0.69876  0.40792 31.17576 

summary(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes) # 43+-28.65 
sd( (fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes)) #  28.6543
#    0.00   24.18   38.00   42.80   52.93  217.00  
summary(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes) #
sd(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes)   # 17.6+-5.7
# .02   14.11   16.43   17.69   19.80   78.28 
summary(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes) #
sd(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes)   # 15.7+-4.3

mean(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes)/ mean(fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes) # ratio = 2.42

t.test((fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes),
       (fullPMNLE$Chrom_PPI/fullPMNLE$Chrom_genes))
plot( (fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes), (fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes), xlim=c(0,230), ylim=c(0,220)) + grid()
fullPMNLE4$rate <- fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes

hist(  log2(fullPMNLE4$Chrom_PPI/fullPMNLE4$Chrom_genes)/(fullPMNLE4$Plasmid_PPI/fullPMNLE4$Plasmid_genes), breaks=44, xlim=c(0,4.5)) + grid()

sum(fullPMNLE4$Plasmid_PPI)
sum(fullPMNLE4$Chrom_PPI)

 # indirect connections
summary(fullPMNLE$indirect_all*(fullPMNLE$Plasmid_genes + fullPMNLE$Chrom_genes))
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
 #        0    5855   14787   20009   27771  146082     125 
summary(fullPMNLE$indirect_chrom*fullPMNLE$Chrom_genes) # indirect connections
#     0    8061   16518   21474   29073  146082     149 
...
for (k in 1:length(names6)){ # check 796-798
     name <-names6[k]
     print(k)
     try ( in2 <- data.frame(read.csv(name)) )
     in2 <- in2[,-c(1)]
...
for (k in 1:length(table3$plasmidPPIs)){ # F = 1 - obs/exp
  fp <- table3[k,6]/(table3[k,6]+table3[k,9])    # f(p)  0.008
  fq <- table3[k,9]/(table3[k,6]+table3[k,9])    # f(q)  0.991
  table3$fp2[k] <- 1 - (table3[k,4]/(table3[k,3]+table3[k,4]+table3[k,5]))/(fp*fp) # p-p PPI
  table3$fq2[k] <- 1 - (table3[k,5]/(table3[k,3]+table3[k,4]+table3[k,5]))/(fq*fq) # c-c PPI
  table3$f[k]   <- 1 - (table3[k,3]/(table3[k,3]+table3[k,4]+table3[k,5]))/(2*fp*fq) # p-c PPI
} # 1 - exp/obs

hist(table3$f, breaks=41, col="pink", xlab="Plasmid-chromosome PPI F value per sample",
     main="", ylab="Number of samples") + grid()
hist(table3$fp2, breaks=41, col="blue", xlab="Plasmid-plasmid F value per sample",
     main="", ylab="Number of samples") + grid()
hist(table3$fq2, breaks=41, col="grey", xlab="Chromosome-chromosome F value per sample",
     main="", ylab="Number of samples") + grid()

setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/Key_Results_Figures/")

table3$Plasmid_genes <- "NA"
table3$Chrom_genes <- "NA"
table3$Fraction <- "NA"
table3$cTri <- "NA"
table3$loops <- "NA"
table3$ccs <- "NA"
...
table3$ccs_all <- "NA"
table3$aPMNLE <- "NA"
table3$aPMNLE_all <- "NA"
table3$diffPMNLE <- "NA"
table3$proteins_ccs_all <- "NA"
table3$proteins_ccs_chrom <- "NA"
table3$indirect_all <- "NA"
table3$indirect_chrom <- "NA"
table3$inputProteins <- "NA"

for (j in 1:length(table3$name)){           # for each table3 sample
    table3$name[j] = gsub('_{1}$', '',   table3$name[j], perl=T) 
    table3$PPIs <- table3$plasmidPPIs + table3$chromPPIs
    table3$interactions <- table3$plasmidPPIs + table3$chromPPIs

    for (j2 in 1:length(fullPMNLE$Sample)){   # for each fullPMNLE sample
         if(table3$name[j] == fullPMNLE$Sample[j2]){ # if match
                  print(table3$name[j])
              table3$Plasmid_genes[j] = fullPMNLE$Plasmid_genes[j2]
              table3$Chrom_genes[j] = fullPMNLE$Chrom_genes[j2]
              table3$Fraction[j] = fullPMNLE$Fraction[j2]
              table3$cTri[j] = fullPMNLE$cTri[j2]
              table3$loops[j] = fullPMNLE$loops[j2]
              table3$ccs[j] = fullPMNLE$ccs[j2]
              table3$cTri_all[j] = fullPMNLE$cTri_all[j2]
              table3$loops_all[j] = fullPMNLE$loops_all[j2]
              table3$ccs_all[j] = fullPMNLE$ccs_all[j2]
              table3$aPMNLE[j] = fullPMNLE$aPMNLE[j2]
              table3$aPMNLE_all[j] = fullPMNLE$aPMNLE_all[j2]
              table3$diffPMNLE[j] = fullPMNLE$diffPMNLE[j2]
              table3$proteins_ccs_all[j] = fullPMNLE$proteins_ccs_all[j2]
              table3$proteins_ccs_chrom[j] = fullPMNLE$proteins_ccs_chrom[j2]
              table3$indirect_all[j] = fullPMNLE$indirect_all[j2]
              table3$indirect_chrom[j] = fullPMNLE$indirect_chrom[j2]
              table3$inputProteins[j] = fullPMNLE$inputProteins[j2]      }
    }
}

dim(table3)
dim(fullPMNLE)
write.xlsx(table3, "Table_S3_4432_samples.xlsx")

for (j in 1:length(fullPMNLE$Sample)){           # for each fullPMNLE sample
    for (j2 in 1:length(table3$name)){   # for each table3 sample
         if(table3$name[j2] == fullPMNLE$Sample[j]){ # if match
              table3$name[j2] = gsub('_{1}$', '',   table3$name[j2], perl=T) 
              print(fullPMNLE$Sample[j])
              fullPMNLE$plasmidPPIs[j] = table3$plasmidPPIs[j2]
              fullPMNLE$plasmidchromPPIs[j] = table3$plasmidchromPPIs[j2]
              fullPMNLE$plasmidplasmidPPIs[j] = table3$plasmidplasmidPPIs[j2]
              fullPMNLE$chromPPIs[j] = table3$chromPPIs[j2]
              fullPMNLE$plasmidPPIs_genes[j] = table3$plasmidPPIs_genes[j2]
              fullPMNLE$plasmidchromPPIs_genes[j] = table3$plasmidchromPPIs_genes[j2]
              fullPMNLE$plasmidplasmidPPIs_genes[j] = table3$plasmidplasmidPPIs_genes[j2]
              fullPMNLE$chromPPIs_genes[j] = table3$chromPPIs_genes[j2]
              fullPMNLE$f[j] = table3$f[j2]
              fullPMNLE$fp2[j] = table3$fp2[j2]
              fullPMNLE$fq2[j] = table3$fq2[j2]
          }
    }
}
fullPMNLE = subset(fullPMNLE, select = -c(Plasmid_Plasmid_PPI, Plasmid_PPI,
                        Chrom_PPI, PPIs, interactions) )
dim(fullPMNLE)
write.xlsx(fullPMNLE, "Table_S3_4363_samples.xlsx")

setwd("/Volumes/GoogleDrive/My Drive/TDA_Summer2021/RMD_FILES/")
pdf("Fvalues.S24.2.pdf", width=6, height=6)
plot(table3$fp2, table3$f, col="black", pch=1, cex=0.2,
     xlab="Plasmid-plasmid PPI F value per sample",
     ylab="Plasmid-chromosome PPI F value per sample",
     xlim=c(-59,1), ylim=c(-59,1)) + grid()
dev.off()

gg1 <- ggExtra::ggMarginal(ggplot(table3, aes(x= table3$fp2, y=table3$f)) +
...
gg2 <- ggExtra::ggMarginal(ggplot(table3, aes(x= table3$fq2, y=table3$f)) +
...
hist(table3$f, breaks=71, col=c1, xlab="PPI F value per sample",
...
plasmid_freq <- (table3[,6]/(table3[,6]+table3[,9]))
hets <- (table3[,3]/(table3[,3]+table3[,4]+table3[,5]))
ppp <- seq(0,1,0.001)
homs <- (table3[,4]/(table3[,3]+table3[,4]+table3[,5]))
plot(plasmid_freq, hets, col=alpha("black", 0.4), pch=1, cex=0.3,
     xlab="Plasmid-related protein frequency",
...
chromPPIs   # 46,116,894
chromPPIs_genes   # 1,413,623
chromPPIs/chromPPIs_genes  # 32.6
...
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#  0.0000  0.1630  0.4127  0.4136  0.6279  0.9979 
summary(t_dist)
## Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
# -15.4982  -1.9485  -1.0546  -0.8965  -0.1315  13.4692 
...
replist <- c("dnaa", "dnab", "dnac", "dnad", "dnae", "dnai", "dnag", "grya",
             "gyrb", "liga", "oric", "prii", "pare", "parc", "pola", "polb",
             "polc", "pold", "ssb") # 19 genes
...
for (k in 1:length(files1)){
     ...
     gene_list_rep <- rbind(gene_list_rep, in2[(in2$Gene %in% replist),])
     gene_list_not <- rbind(gene_list_not, in2[!(in2$Gene %in% replist),])
} # end for

dim(gene_list_rep)
dim(gene_list_not)
1 - dim(gene_list_rep)[1]/(dim(gene_list_rep)[1] + dim(gene_list_not)[1])
gene_list_rep1 <- subset(gene_list_rep, Plasmid==1)$Interactions # plasmid-chr PPIs
sd(gene_list_rep1) # 50.3
summary(gene_list_rep1) #  mean=97.8, median=93
gene_list_rep0 <- subset(gene_list_rep, Plasmid==0)$Interactions # chr-chr PPIs
sd(gene_list_rep0) # 104.0
summary(gene_list_rep0) # mean=104.1, median=81
View(gene_list_rep)
hist(gene_list_rep0, col="blue")
hist(gene_list_rep1, col="red")

length(gene_list_rep$Interactions) # 104
length(gene_list_not$Interactions) # 38.4

gene_list_rep1n <- subset(gene_list_not, Plasmid==1)$Interactions # plasmid-chr PPIs
sd(gene_list_rep1n) # 510
summary(gene_list_rep1n) #  mean=78, median=31
gene_list_rep0n <- subset(gene_list_not, Plasmid==0)$Interactions # chr-chr PPIs
sd(gene_list_rep0n) # 284
summary(gene_list_rep0n) # mean=38, median=15
