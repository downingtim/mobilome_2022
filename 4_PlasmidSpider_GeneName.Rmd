...
hist( plot1, breaks=121, col="green", xlim=c(0,120), density =42, ylim=c(0,100),
...
hist.data$counts = as.numeric(log10(hist.data$counts )) 
 hist.data$counts[is.na(hist.data$counts)] <- 0
  hist.data$counts[is.infinite(hist.data$counts)] <- 0
plot( hist.data,   col="purple", main="",  xlim=c(0,115), # xaxt="n", yaxt="n",
     ylab="Log10-scaled number of proteins", density =42, cex.lab=1.3,
     xlab="Average number of PPIs per protein per sample")  
hist.data = hist(  (fullPMNLE$Plasmid_PPI/fullPMNLE$Plasmid_genes), plot=F,
                   breaks=111)
hist.data$counts = as.numeric(log10(hist.data$counts )) 
 hist.data$counts[is.na(hist.data$counts)] <- 0
  hist.data$counts[is.infinite(hist.data$counts)] <- 0
plot( hist.data,   col="green", add=T, cex.lab=1.3, density =34) + grid()
...
for (j in 1:length(fullPMNLE$name)){           # for each fullPMNLE sample

    for (j2 in 1:length(table3$Sample)){   # for each table3 sample
...
write.xlsx(table3, "Table_S3_4363_samples.xlsx")
